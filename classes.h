...
public:
    ...
    void write_into_data_file(ostream& out) const { 
        ...
        for (const auto& slots : slot_directory) { // *TODO: Write the slot-directory information into page_data. You'll use slot-directory to retrieve record(s).
            // slot fields are structured: record pointer, record size
            memcpy(page_end - offset - sizeof(int)*2, &slots.first, sizeof(int));
            memcpy(page_end - offset - sizeof(int), &slots.second, sizeof(int));
            offset += sizeof(int)*2;
        }
